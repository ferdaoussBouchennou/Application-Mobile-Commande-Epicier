/**
 * In-memory cart store (session-style). One cart per client_id.
...
function findItemIndex(clientId, produitId) {
  const items = getItems(clientId);
  return items.findIndex((i) => Number(i.produit_id) === Number(produitId));
}
...
function addItem(clientId, produit_id, quantite = 1, epicier_id = null) {
  ...
  return existing ? existing.quantite : list.find((i) => Number(i.produit_id) === Number(produit_id)).quantite;
}
...
/** Remove from cart the items that were used for this order (by produit_id). */
